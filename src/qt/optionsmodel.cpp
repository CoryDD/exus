...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeExusAmount"))
        settings.setValue("nAnonymizeExusAmount", 1000);
    nAnonymizeExusAmount = settings.value("nAnonymizeExusAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeExusAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeExusAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeExusAmount:
            return QVariant(nAnonymizeExusAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeExusAmount:
            nAnonymizeExusAmount = value.toInt();
            settings.setValue("nAnonymizeExusAmount", nAnonymizeExusAmount);
            emit AnonymizeExusAmountChanged(nAnonymizeExusAmount);
            break;
        default:
            break;
        }

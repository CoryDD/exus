...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeExusAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeExusAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeExusAmount = strAnonymizeExusAmount.remove(strAnonymizeExusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
    ...
    if(nMaxToAnonymize > nAnonymizeExusAmount*COIN) nMaxToAnonymize = nAnonymizeExusAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeExusAmount * COIN) {

...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Exus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Exus
    // Mac: ~/Library/Application Support/Exus
    // Unix: ~/.exus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Exus";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "exus.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "exusd.pid"));

...
Copyright (c) 2009-2012 Exus developers
...
/**
 * Bootup the masternode, look for a 5000 EXUS input and register on the network
 */
void CActiveMasternode::ManageStatus() {
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in exus.conf */
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress) {
    CExuscoinAddress address(collateralAddress);
